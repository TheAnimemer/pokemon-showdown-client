	...
	load: function (bgUrl, bgid, hues) {
		...
		if (!bgid) {
			if (location.host === 'smogtours.psim.us') {
				...
				bgid = ['horizon', 'ocean', 'waterfall', 'shaymin', 'tanoso', 'psday'][Math.floor(Math.random() * 6)];
			} else {
				$(document.body).css({
					background: '',
					'background-size': ''
		...
		if (!hues) {
			...
			case 'tanoso':
				...
				attrib = '<a href="https://lit.link/en/seiryuuden" target="_blank" class="subtle">"tanoso" <small>background by Jessica Valencia</small></a>';
				break;
			case 'psday':
