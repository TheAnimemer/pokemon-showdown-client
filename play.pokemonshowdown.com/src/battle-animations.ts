...
export class BattleScene implements BattleSceneStub {
	...
	typeAnim(pokemon: Pokemon, types: string) {
		const result = BattleLog.escapeHTML(types).split('/').map(type =>
			`<img src="${Dex.resourcePrefix}sprites/newTypes/${encodeURIComponent(type)}.png" alt="${type}" class="pixelated" />`
		).join(' ');
...
export class PokemonSprite extends Sprite {
	...
	getStatbarHTML(pokemon: Pokemon) {
		...
		if (pokemon.terastallized) {
			buf += ` <img src="${Dex.resourcePrefix}sprites/newTypes/Tera${pokemon.terastallized}.png" alt="Tera-${pokemon.terastallized}" style="vertical-align:text-bottom;" height="16" width="16" />`;
		}
	...
	updateStatbar(pokemon: Pokemon, updatePrevhp?: boolean, updateHp?: boolean) {
		...
		if (pokemon.terastallized) {
			status += `<img src="${Dex.resourcePrefix}sprites/newTypes/${encodeURIComponent(pokemon.terastallized)}.png" alt="${pokemon.terastallized}" class="pixelated" /> `;
		} else if (pokemon.volatiles.typechange && pokemon.volatiles.typechange[1]) {
			const types = pokemon.volatiles.typechange[1].split('/');
			for (const type of types) {
				status += '<img src="' + Dex.resourcePrefix + 'sprites/newTypes/' + encodeURIComponent(type) + '.png" alt="' + type + '" class="pixelated" /> ';
			}
		}
		if (pokemon.volatiles.typeadd) {
			const type = pokemon.volatiles.typeadd[1];
			status += '+<img src="' + Dex.resourcePrefix + 'sprites/newTypes/' + type + '.png" alt="' + type + '" class="pixelated" /> ';
		}
