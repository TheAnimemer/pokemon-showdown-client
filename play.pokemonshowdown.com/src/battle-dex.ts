...
*/
	...
	getPokemonIcon(pokemon: string | Pokemon | ServerPokemon | Dex.PokemonSet | null, facingLeft?: boolean) {
		...
		return `background:transparent url(https://raw.githubusercontent.com/TheAnimemer/pokemon-sprites/refs/heads/main/sprites/types/pokemonicons-sheet.png) no-repeat scroll -${left}px -${top}px${fainted}`;
	}
	...
	getTypeIcon(type: string | null, b?: boolean) { // b is just for utilichart.js
		...
		return `<img src="${Dex.resourcePrefix}sprites/newTypes/${sanitizedType}.png" alt="${type}" height="14" width="32" class="pixelated${b ? ' b' : ''}" />`;
	}
